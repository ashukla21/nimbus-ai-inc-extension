...
class Database:
    ...
    def __init__(self):
        ...
        df = pd.read_csv('core/stocks/data_fetch/robinhood_data/100_most_popular_cleaned.csv')
        # reading in csv file of stock ticker names
        df_names = pd.read_csv('core/stocks/data_fetch/robinhood_data/100_most_popular_names.csv')
        df_names.drop(columns=['Unnamed: 0'], inplace=True)
        userName = None
        passWord = None

        # get login in info for database
        with open('core/stocks/LoginCredentials/mysql3.txt') as f:
            file = f.readlines()
